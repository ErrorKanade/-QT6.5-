...
public:
	...
	int openPort();                     //打开串口
	int closePort();                    //关闭串口
	int showLogInfo(const QString& mag);       //打印日志
	int sendMessage();                  //发送消息
	int receiveData();                   //接收数据槽函数
	int saveData();                     //保存数据
	int realTimeSaveData(const QString& path); //实时保存数据
	int getTime();                      //获取系统时间
...
private slots:
    ...
    void on_Lagbox_currentTextChanged(const QString &arg1);
