...
{
    //设置标题、图标
    this->setWindowTitle("Pin脚快速检测系统");
    this->setWindowIcon(QIcon(":/img/img/KPD.ico"));
    //初始化状态栏
    initStatusBar();
    //初始化设置选项
    initSettings();
...
int MainWindow::initSettings()
{
    ...
    //设置语言
    ui->Lagbox->addItem("简体中文");
    ui->Lagbox->addItem("English");
    ui->Lagbox->addItem("繁体中文");
    ui->Lagbox->setCurrentIndex(0);

    //设置可选选项
    ui->timeCheckBox->setChecked(false);
    ui->newLineCheckBox->setChecked(false);
...
void MainWindow::SetValue(QString value) {
...
void MainWindow::on_Lagbox_currentTextChanged(const QString &arg1)
{
    static QTranslator translator; // 靜態變量儲存翻譯器物件
    qApp->removeTranslator(&translator); // 移除先前的翻譯器
    QString lang; // 记录语言代号ui->Lagbox->currentIndex()

    switch (ui->Lagbox->currentIndex())
    {
    case 0:
        lang = "zh_CN";
        qDebug() << "Selected value: " << ui->Lagbox->currentIndex();
        break;
    case 1:
        lang = "English";
        qDebug() << "Selected value: " << ui->Lagbox->currentIndex();
    ...
    qDebug() << "Trying to load translation file:" << ":/translations/lang_" + lang + ".qm";
    if (translator.load(":/translations/lang_" + lang + ".qm")) {
        qDebug() << "Translation loaded successfully.";
        qApp->installTranslator(&translator);
        ui->retranslateUi(this);
        qDebug() << "Selected language:" << arg1;
    } else {
